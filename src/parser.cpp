        ...
        while (true) {
            ...
            while (!check(TokenType::Comma) && !check(TokenType::Semicolon) && !check(TokenType::Eof)) {
                if (match(TokenType::Fixed)) {
                    ...
                    // BINARY/DECIMAL are base attributes that modify whichever
                    // of FIXED/FLOAT was already parsed (they can appear in
                    // either order in real PL/I, e.g. both `FLOAT BINARY` and
                    // `BINARY FLOAT` mean the same thing) — never downgrade an
                    // already-parsed FLOAT back to fixed-point.
                    if (data_type != "FLOAT") data_type = "FIXED BIN";
                    if (match(TokenType::LParen)) { while (!match(TokenType::RParen)) advance(); }
